//
//  StoreAsyncFetcher.swift
//  DAWG
//
//  Created by Simon Hogg on 2020-04-02.
//  Copyright © 2020 Simon Hogg. All rights reserved.
//

import Foundation

/// - Tag: StoreAsyncFetcher
class StoreAsyncFetcher {
    // MARK: Types
    ...
    private var completionHandlers = [UUID: [(Store?) -> Void]]()

    /// An `NSCache` used to store fetched objects.
    private var cache = NSCache<NSUUID, Store>()
    ...
    */
    func fetchAsync(_ identifier: UUID, completion: ((Store?) -> Void)? = nil) {
    ...
    /**
     ...
     - Returns: The 'Store' that has previously been fetched or nil.
     */
    func fetchedData(for identifier: UUID) -> Store? {
     ...
     */
    private func invokeCompletionHandlers(for identifier: UUID, with fetchedData: Store) {
...
class AsyncFetcherOperation: Operation {
    ...
    private(set) var fetchedData: Store?
    ...
    override func main() {
        ...
        fetchedData = Store.allStores.filter({$0.uuid == identifier})[0]
    }
}
